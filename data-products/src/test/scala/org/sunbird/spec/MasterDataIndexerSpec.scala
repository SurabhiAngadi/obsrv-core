...
class MasterDataIndexerSpec extends FlatSpec with BeforeAndAfterAll with Matchers {
  ...
  it should "return null when datasource is empty" in {
    val dataset = DatasetRegistry.getDataset("md4")
    val provider = jobConfig.withValue("cloudStorage.container", ConfigValueFactory.fromAnyRef(s"${pwd}/obsrv-data"))
    val filteredDatasource = CommonUtils.fetchDatasource(dataset.get)
    MasterDataProcessorIndexer.processDatasets(provider)
    assert(filteredDatasource == null)
    verify(mockMetrics).generate(datasetId = dataset.get.id, edata = Edata(metric = mutable.Map(mockMetrics.getMetricName("failure_dataset_count") -> 1), labels = List(MetricLabel("job", "MasterDataIndexer"), MetricLabel("datasetId", dataset.get.id), MetricLabel("cloud", s"${jobConfig.getString("cloudStorage.provider")}")), err = "Failed to index dataset.", errMsg = "Dataset should have single datasource."))
  }
